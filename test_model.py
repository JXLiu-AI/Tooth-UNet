...
import os
from nets.ToothUNet import ToothUNet
from utils import *
import cv2
...
def vis_and_save_heatmap(name, model, input_img, img_RGB, labs, vis_save_path, dice_pred, dice_ens):
    model.eval()
    ...
    ################
    pred_class = torch.where(output>0.8,torch.ones_like(output),torch.zeros_like(output))
    predict_save = pred_class[0].cpu().data.numpy()
    predict_save = np.reshape(predict_save, (config.img_size, config.img_size))
    # dice_pred_tmp, iou_tmp = show_image_with_dice(predict_save, labs, save_path=vis_save_path+'_predict'+model_type+'.png')
    # return dice_pred_tmp, iou_tmp
    #########################
    base, houzui = os.path.splitext(name)
    cv2.imwrite('/home/jiaxiang/UCTransNet-main/results/predict_pro_real/' + base + '.png', predict * 255)
    return 1,1
    ##########################
...
if __name__ == '__main__':
    ...
    elif config.task_name is "tooth":
        test_num = 1039
        model_type = config.model_name
        model_path = "./tooth/"+model_type+"/"+test_session+"/models/best_model-"+model_type+".pth.tar"
    ...
    if model_type == 'ToothUNet':
        config_vit = config.get_ToothTransformer_config()
        model = ToothUNet(config_vit,n_channels=config.n_channels,n_classes=config.n_labels)

    elif model_type == 'ToothUNet_pretrain':
        config_vit = config.get_ToothTransformer_config()
        model = ToothUNet(config_vit,n_channels=config.n_channels,n_classes=config.n_labels)
    ...
    with tqdm(total=test_num, desc='Test visualize', unit='img', ncols=70, leave=True) as pbar:
        for i, (sampled_batch, names) in enumerate(test_loader, 1):
            ...
            name = names[0]
            base, houzui = os.path.splitext(name)
            cv2.imwrite('/home/jiaxiang/UCTransNet-main/results/GT/' + base + '.png', img_lab)
            ...
            plt.subplots_adjust(top=1, bottom=0, left=0, right=1, hspace=0, wspace=0)
            plt.margins(0, 0)
            plt.savefig(vis_path+str(i)+"_lab.png", dpi=300)
            plt.close()
            input_img = torch.from_numpy(arr)
            dice_pred_t,iou_pred_t = vis_and_save_heatmap(name, model, input_img, None, lab,
                                                          vis_path+str(i),
