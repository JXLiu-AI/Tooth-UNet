...
class Tooth_Embeddings(nn.Module):
...
class ToothAttention(nn.Module):
...
class Block_ViT(nn.Module):
    def __init__(self, config, vis, channel_num):
        ...
        self.channel_attn = ToothAttention(config, vis, channel_num)
...
class ToothTransformer(nn.Module):
    def __init__(self, config, vis, img_size, channel_num=[64, 128, 256, 512], patchSize=[32, 16, 8, 4]):
        ...
        self.embeddings_1 = Tooth_Embeddings(config,self.patchSize_1, img_size=img_size,    in_channels=channel_num[0])
        self.embeddings_2 = Tooth_Embeddings(config,self.patchSize_2, img_size=img_size//2, in_channels=channel_num[1])
        self.embeddings_3 = Tooth_Embeddings(config,self.patchSize_3, img_size=img_size//4, in_channels=channel_num[2])
        self.embeddings_4 = Tooth_Embeddings(config,self.patchSize_4, img_size=img_size//8, in_channels=channel_num[3])
